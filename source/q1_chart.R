...
create_scatter <- function(data, variable, labels) {
  ggplot(data = data, mapping = aes(x = get(variable),
    ...
    theme(plot.title = element_text(hjust = 0.5, face = "bold"),
          ...
          axis.title.x =
            element_text(margin = margin(t = 15, r = 0, b = 0, l = 0)),
          axis.title.y =
            element_text(margin = margin(t = 0, r = 15, b = 0, l = 0)),
          panel.border =
            element_rect(colour = "black", fill = NA, size = 3))
}

# Function to create labels
create_speed_labels <- function(speed) {
  title <- paste("How does having", speed,
                "Internet speed affect high school performance?")
  x <- paste("County proportion with", speed,
            "Internet speed")
  c(title, x)
}
...
summarize_data <- function(data) {
  ...
  group_by(County, percent_has_1_medium, percent_has_1_fast, prop_1_2_medium,
           prop_1_2_fast) %>%
